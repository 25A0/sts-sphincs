...
static void chacha12_wordtobyte(byte output[64],const u32 input[16])
{
  u32 x[16];
  int i;
...
{
  u32 x[16];
  chacha_keysetup(x, key, crypto_stream_chacha12_KEYBYTES * 8, crypto_stream_chacha12_NONCEBYTES * 8);
  chacha_ivsetup(x, nonce);
  chacha_keystream_bytes(x, r, rlen);
}
